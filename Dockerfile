...
RUN apt-get install -y unzip python3-pip wget lsb-core && mkdir /root/.aws && echo region = us-east-1 > /root/.aws/credentials && pip3 install awscli
...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y postgresql-10
